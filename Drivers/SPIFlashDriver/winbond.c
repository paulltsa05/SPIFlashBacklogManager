...
#endif
				//PRINTF("PP: 0x%p => cmd = { 0x%02x 0x%02x%02x%02x } chunk_len = %zu\n\r", buf + actual,
				//	cmd[0], cmd[1], cmd[2], cmd[3], chunk_len);
...
#endif
	//PRINTF("SF: Winbond: Successfully programmed %zu bytes @ 0x%lx\n\r or sector %d ", len, (unsigned long)(offset - len),offset/4096);
	ret = 0;
