...
extern SPI_HandleTypeDef hspi2; //used SPI define by CubeMX
...
{
	int ret = spi_xfer(spi, &cmd, 1, response, len );
	if (ret)
		__NOP();
...
{   int ret;
	...
	//HAL_SPI_TransmitReceive(&hspi2,(uint8_t *)dout,(uint8_t *)din,bytesout,50);
   ret= HAL_SPI_Transmit(&hspi2, (uint8_t *)dout, bytesout, 10);
   if(ret == HAL_OK && bytesin !=0)
	   ret=HAL_SPI_Receive(&hspi2, (uint8_t *)din, bytesin, 10);
